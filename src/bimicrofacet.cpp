...
class BiMicrofacet : public BSDF {
public:
    BiMicrofacet(const PropertyList &propList) {
        ...
        /* To ensure energy conservation, we must scale the 
           ...
           of this BRDF. */
        m_ks = propList.getFloat("ks", 0.5f);
        m_kd *= (1.0f - m_ks);
    }
    ...
    Color3f eval(const BSDFQueryRecord &bRec) const {
        ...
        res *= fresnel(wh.dot(wi), m_extIOR, m_intIOR);
        res *= G1(wi, wh) * G1(wo, wh);
        res /= 4.0f * Frame::cosTheta(wi) * Frame::cosTheta(wo) * Frame::cosTheta(wh);
    ...
    float pdf(const BSDFQueryRecord &bRec) const {
        ...
        res /= 4.0f * wh.dot(wo);
        res += (1.0f - m_ks) * Frame::cosTheta(wo) * INV_PI;
        return res;
    }
    ...
    Color3f sample(BSDFQueryRecord &bRec, const Point2f &_sample) const {
        ...
        if (Frame::cosTheta(bRec.wi) < 0.0f)
            flip = true;
        else if (Frame::cosTheta(bRec.wi) == 0.0f)
            return Color3f(0.0f);
...
NORI_REGISTER_CLASS(BiMicrofacet, "bimicrofacet");
NORI_NAMESPACE_END
